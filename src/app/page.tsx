...
export default function Home() {
  ...
  const refreshPickupsForRole = useCallback(async (offset: number = 0, append: boolean = false) => {
    ...
    if (me.role === "collector") {
      ...
      try { 
        const data = await fetchJSON<Pickup[]>(`/api/pickups?offset=${offset}`); 
        setPickups(prev => append ? [...prev, ...data] : data); 
      } catch {}
      return;
    }
                              ...
                              <div className="flex items-center gap-4 text-sm text-gray-600">
                                ...
                                {p.assigned_collector_id && (
                                  <span className="flex items-center gap-1">
                                    ...
                                    {p.assigned_collector_name && p.assigned_collector_name.trim() !== '' ? p.assigned_collector_name : `Collector #${p.assigned_collector_id}`}
                                  </span>
                                )}
                                <span className="flex items-center gap-1">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
