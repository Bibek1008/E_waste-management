...
export default function Home() {
  ...
  const refreshPickupsForRole = useCallback(async () => {
    if (!me) return;
    if (me.role === "resident") {
      try { setPickups(await fetchJSON<Pickup[]>(`/api/pickups?role=resident&user_id=${me.id}`)); } catch {}
      return;
    }
    if (me.role === "collector") {
      // Collectors need to see all pickups to view available ones for assignment
      try { setPickups(await fetchJSON<Pickup[]>(`/api/pickups`)); } catch {}
      return;
    }
    try { setPickups(await fetchJSON<Pickup[]>(`/api/pickups`)); } catch {}
  }, [me]);
                    ...
                    pickups.filter(p => p.status === "pending" && !p.assigned_collector_id).map(p => (
                      <div key={p.id} className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-6 hover:shadow-lg transition-all">
                        ...
                        <div className="flex gap-3">
                          <button
                            className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex items-center justify-center gap-2"
                            onClick={async () => {
                              try {
              ...
              <section className="lg:col-span-3 bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-emerald-100 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-3xl p-6 text-white shadow-xl">
                    ...
                    <div className="text-3xl font-bold mb-1">{pickups.filter(p => p.assigned_collector_id === me?.id).length}</div>
                    <div className="text-emerald-100">Total Assigned</div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-3xl p-6 text-white shadow-xl">
                    ...
                    <div className="text-3xl font-bold mb-1">{pickups.filter(p => p.assigned_collector_id === me?.id && p.status === 'completed').length}</div>
                ...
                <div className="mt-8 bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-6 border border-gray-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <button 
                      className="bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold py-3 px-4 rounded-xl shadow-lg transition-all flex items-center gap-2"
                    ...
                    <button className="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white font-semibold py-3 px-4 rounded-xl shadow-lg transition-all flex items-center gap-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-semibold py-3 px-4 rounded-xl shadow-lg transition-all flex items-center gap-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
